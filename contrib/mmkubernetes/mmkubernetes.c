...
static rsRetVal
extractMsgMetadata(msg_t *pMsg, uchar *propName, regex_t *fnRegex,
...
CODESTARTdoAction
	...
	if(jMetadata == NULL) {
		...
		if(jo2 == NULL) {
			...
			if(RS_json_object_object_get_ex(jReply, "metadata", &jo2)
				&& RS_json_object_object_get_ex(jo2, "uid", &jo2))
		...
		if(RS_json_object_object_get_ex(jReply, "nodeName", &jo2))
			json_object_object_add(jo, "host", json_object_get(jo2));
		if(RS_json_object_object_get_ex(jReply, "uid", &jo2))
